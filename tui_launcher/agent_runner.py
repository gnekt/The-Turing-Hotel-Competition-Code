...
from run import SETUP_FILE, node_name_for, run_agent
...
def main():
    ...
    setup_file = os.environ.pop("COMPETITION_SETUP_FILE", str(SETUP_FILE))
    ...
    with open(setup_file, newline="", encoding="utf-8") as file:
        config = next(row for row in csv.DictReader(file) if row["id"] == agent_id)

    print(f"[launcher] Starting {node_name_for(config)} (PID {os.getpid()})", flush=True)
    try:
